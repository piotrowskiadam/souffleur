...
function handleAddPromptToggle() {
  ...
  if (!isAddingPrompt) {
    ...
    if (title && text) { // Ensure both fields have content
      ...
      promptList.unshift({ title, text }); // Consider adding a unique ID here if needed later
  ...
  if (file) {
    ...
    reader.onload = (e) => {
      try {
        ...
        promptList = importedPrompts; // Replace current list
        ...
        alert(
          "Error importing prompts. Please make sure the file contains a valid JSON array of prompts."
        );
      } finally {
...
function handleExportClick() {
  try {
    const dataStr = JSON.stringify(promptList, null, 2); // Pretty print JSON
    const dataUri =
